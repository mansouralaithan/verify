...
#  systemctl is-enabled "${service}" 2>&1 | egrep -q 'disabled|Failed' || return
  chkconfig --list | grep $(runlevel | awk '{ print $2}'):on |  grep "${service}" 2>&1 && return 1 || return 0
}

test_service_enabled() {
  local service="$1" 
  chkconfig --list | grep $(runlevel | awk '{ print $2}'):on |  grep "${service}" 2>&1 && return 0 || return 1
}
